...
var startAudio = function (currentTime, fromCache) {
	...
	if (!fromCache) {
		...
		root.playing = true;
		root.data = root.data!=null?root.data:root.replayer.getData();
		root.endTime = getLength();
	}
	var note;
	var offset = 0;
...
root.toggleNote = function (noteInfo, measureDuration) {
	var noteFound = false;
	var noteIndex = 0;
	var data = root.data;
	var currentTime = 0.0;
	...
	{
		var noteData = data[n];
		currentTime += noteData[1];
		event = noteData[0].event;
		if(currentTime < noteInfo.startTime)
		{
			noteIndex++;
			lastTime = currentTime;
		}
		else if(event.subType == 'noteOn' && currentTime >= noteInfo.startTime && currentTime <= noteInfo.startTime + noteInfo.duration && event.noteNumber == noteInfo.noteNumber)
		{
			noteFound = true;
			data.splice(n,1);
		}
	...
	if(!noteFound)
	{
		...
		console.log("Insert note: " + noteInfo.noteNumber + ", at index: " + noteIndex + ", at time: " + noteInfo.startTime);
		data.splice(noteIndex, 0, [{event:event, track:0 /*, ticksToEvent:100.0*/}, noteInfo.startTime-lastTime, "ADDED"]);
		//
		// var note = event.noteNumber - (root.MIDIOffset || 0);
		//
		// var channel = 0
		// eventQueue.splice(noteIndex, 0, {
		// 	event: event,
		// 	source: MIDI.noteOn(channel, event.noteNumber, event.velocity, noteInfo.startTime / 1000.0),
		// 	interval: null // scheduleTracking(channel, note, queuedTime, offset, 144, event.velocity)
		// });
	}
}
